...
Model (double X[], double Y[], int n ) : n(n), X(X), Y(Y)    { ////////////////////////////////   CTOR
	...
	C_step.assign(0.1);     C_step[0]= 0.5;      C_step[1]=0.2;
...
double  inverse_estimate (const double cdf)    const {
	...
	double r_guess = 3;
	double cdf_guess = estimate(r_guess);
	if (cdf_guess < cdf)	r_step =  1.0;
	else			r_step = -1.0;
	...
	LOOP:
		...
		r_guess += r_step;
					assert(r_guess > -25);
					assert(r_guess < 30);
		cdf_guess = estimate(r_guess);
	if (r_step > 0   &&  cdf_guess < cdf) goto LOOP;
	if (r_step < 0   &&  cdf_guess > cdf) goto LOOP;
	


	////// binary(?) search 
	double       r_low     = min(r_guess, r_last);
        double       r_high    = max(r_guess, r_last);
...
void	print (string title="")  const {
	cout << "# :gnuplot: set key bottom; set ytics 0,0.5; set yrange [-0.1:1.1]; set xrange [-2:7]; set grid; set pointsize 1.5;"
	...
	for (double x=-15.0;  x<=16.0; x+=1./3.	)	cout << table	% x   	% _		% estimate(x)	% _  ;
	for (double y=0.01;  y<=0.99; y+=0.05	)	cout << table	% _ 	% y   		% _    		% inverse_estimate(y) ;
 };
