...
double f(const gsl_vector * v, void *params) {
	/* Paraboloid centered on (p[0],p[1]), with  scale factors (p[2],p[3]) and minimum p[4] */
	const double  *x = gsl_vector_const_ptr(v, 0);
	double        *p = (double *) params;
	return p[2] * (x[0] - p[0]) * (x[0] - p[0]) + p[3] * (x[1] - p[1]) * (x[1] - p[1]) + p[4];
}
...
int	main(void)  {
	...
	cout << 
		"# :gnuplot:  set view 0,90,1.7;  set dgrid3d;  set key off;  set contour surface;  set cntrparam levels 20;  set isosample 20;"
	...
	FMinimizer fm(2, f, x, par, ss, true);	
	fm.iterate(1e-2, 100);
